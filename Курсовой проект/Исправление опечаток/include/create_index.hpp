...
bool merge_blocks(path path2blocks, path path2index)
{
    ...
    */
    ...
    SETUP_PROGRESS(terms_total, &current_sec, L"Слияние термов");
    while (terms_current > 0)
    {
        ...
        n_terms++;
        UPDATE_PROGRESS(terms_total - terms_current, terms_total, &current_sec, L"Слияние термов");
    }
                ...
                dict<wstring, item> &terms, uint *num_docs = NULL)
{
    ...
    for (i = 0; i < n_terms; i++)
    {
        ...
        UPDATE_PROGRESS(i + 1, n_terms, &current_sec, L"Чтение термов");
    }
...
bool calculate_doc_tf(const path &path2index)
{
    ...
    SETUP_PROGRESS(n_terms, &current_sec, L"Первый проход");
    for (i = 0; i < n_terms; i++)
    {
        ...
        UPDATE_PROGRESS(i + 1, n_terms, &current_sec, L"Первый проход");
    }
    ...
    SETUP_PROGRESS(tf.size(), &current_sec, L"Второй проход");
    for (i = 0; i < tf.size(); i++) // цикл по документам
    {
        ...
        UPDATE_PROGRESS(i + 1, tf.size(), &current_sec, L"Второй проход");
    }
