...
LRESULT CALLBACK ParamsProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_COMMAND:
        ...
        case ParamsPath2CorpusButtonClicked:
            if (BrowseForFolder(hWnd, L"Выберите директорию с корпусом", fs::current_path().c_str(), buf.data()))
            {
        ...
        case ParamsPath2IndexButtonClicked:
            if (BrowseForFolder(hWnd, L"Выберите директорию для хранения индекса", fs::current_path().c_str(), buf.data()))
            {
        ...
        case ParamsPath2BlocksButtonClicked:
            if (BrowseForFolder(hWnd, L"Выберите директорию для хранения временных файлов", fs::current_path().c_str(), buf.data()))
            {
...
bool BrowseForFolder(HWND hwnd, const wchar *title, const wchar *folder, wchar *select_folder)
{
    BROWSEINFOW br = {0};
