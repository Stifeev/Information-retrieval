...
def main():
    ...
    if len(sys.argv) <= 4:
        is_lemma = True if sys.argv[3] == "-lemma" else False
    ...
    while i < len(tokens):
        ...
        if is_lemma:
            ...
            if tokens[i].lemma in {"(", ")", "!", "/", "\""}:
                ...
                fail = False
                for letter in tokens[i].text:
                    if not (letter.isalpha() or letter.isdigit() or letter == '-'):
                        fail = True
                        break
                
                if not fail:
                    request.append(tokens[i].text)
            
        i += 1
    
    fuzzy = False
    if len(request) > 1 and {"(", ")", "!", "/", "\"", "&", "|"} & set(request) == set(): # проверка на начёткий поиск
        fuzzy = True
