                     ...
                     vector<int> &starts, vector<int> &ends, vector<int> &work)
{
    ...
    */
    ...
    if (work[i] != doc_id) // документ не найден
    {
        starts.resize(0);
        ends.resize(0);
        return;
    ...
    uint offset;
    fseek(fp_posting, (n_docs - i - 1) * sizeof(int) + i * sizeof(uint), SEEK_CUR);
    fread(&offset, sizeof(uint), 1, fp_posting);
    fseek(fp_posting, (n_docs - i - 1) * sizeof(uint) + offset * sizeof(int), SEEK_CUR);
    starts.resize(freq);
    ends.resize(freq);
...
int get_term_freq_in_doc(wstring &term, int doc_id,
                         const dict<wstring, uint> &terms, FILE *fp_posting,
                         vector<int> &work)
{
...
void get_terms_weight_in_doc(const path &path2index, int doc_id, const vector<wstring> &id2term,
                             dict<wstring, double> &terms, vector<int> &iwork, vector<double> &work)
{
    terms.clear();
    ...
    int *term_id = iwork.data();
    double *weights = work.data();
    fread(term_id, sizeof(int), n_terms, fp);
    fread(weights, sizeof(double), n_terms, fp);
    int i;
    for (i = 0; i < n_terms; i++)
    {
        terms[id2term[term_id[i]]] = weights[i];
    }
